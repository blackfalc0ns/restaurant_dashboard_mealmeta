...
@Component({
  ...
  template: `
    ...
    >
      ...
      <ng-icon
        name="lucideCheck"
        class="pointer-events-none relative z-20 text-white transition-opacity [&_svg]:size-full [&_svg]:stroke-[3] [&_svg]:stroke-white [&_svg]:text-white"
...
export class ZardCheckboxComponent implements ControlValueAccessor {
  ...
  protected readonly boxClasses = computed(() => (this.zSize() === 'lg' ? 'size-6' : 'size-4'));

  protected readonly iconSizeClasses = computed(() => (this.zSize() === 'lg' ? 'size-3.5' : 'size-2.5'));
